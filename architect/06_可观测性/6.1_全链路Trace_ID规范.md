...
| `span_id` | `string` | 选填 | **跨链 Saga 强制约束：** 同一个跨链调度任务（如 Arbitrum → Mainnet）必须共享同一个 `trace_id`。发出链（Hop-1）的操作标为 `span_id: hop-1`，目标链到账（Hop-2）标为 `span_id: hop-2`。以此精准定位 30 分钟超时是卡在发送端还是跨链桥。 |
