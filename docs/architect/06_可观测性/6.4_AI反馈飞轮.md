...
**RAG (Retrieval-Augmented Generation) 机制实现长期记忆：**
大语言模型本身是无状态的。本系统将 Vector DB 作为 AI Agent 的“长期记忆体”。每次生成优化 Prompt 前，必须先用当前的市场特征（如“高波动+流动性枯竭”）向量化后去 Vector DB 检索历史失败的 `trace_id` 及复盘经验。将检索结果（相似失败案例）作为 Context 注入 Prompt，真正让飞轮“记住”昨天的教训。
