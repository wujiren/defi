...
### 3.4 Kill Switch 绿色通道 (Fast-Track)
面对黑客攻击，若全局断路器（§5.3）触发的紧急撤资（Revoke / Withdraw）因为超过 $10,000 而进入 `PENDING_APPROVAL`，将导致毁灭性的死锁。
**规则约束：**
- 由 Admin Web Portal 特定高权限接口触发的、带有 `IS_KILL_SWITCH=TRUE` 标记的 Payload，**无视任何大额审批阈值限制**。
- Whitelist Proxy 在验证该签名请求确属紧急提款白名单函数后，直接放行进入 MPC 签名，实现秒级落块。
...
1. FSM 推进至 SIGNED 前：
   - 获取 Nonce 锁
   - 构建 unsigned Payload（包含 to, data, value, nonce, chainId）
   - 将未签名的**明文 Payload**（而非 Hash）通过 mTLS 发送至 Zone C

2. Zone C（Whitelist-Enforcement-Proxy）接收：
   - 校验 mTLS 客户端证书 + 来源 IP
   - 基于明文 Payload 执行 §2 的白名单和函数选择器校验
   - 校验通过后，由 Zone C 内部自行对明文计算 Keccak256 Hash
   - 签名完成后，由 Zone C 返回签名结果至 Zone B
