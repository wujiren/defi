...
COMMENT ON TABLE  trade_tasks IS 'FSM 核心状态表，每笔交易对应一行，状态变更必须与此表事务原子提交';
...
COMMENT ON TABLE  nonce_locks IS '分布式 Nonce 锁，通过 SELECT FOR UPDATE 实现串行分配';
...
COMMENT ON TABLE ledger_entries IS '复式记账凭证表，每笔资产变动必须同时生成借贷两条分录，金额相等';
```
