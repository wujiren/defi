...
| 状态 | 进入条件 | 执行动作与持久化 | 崩溃自愈逻辑 |
|---|---|---|---|
| **INIT** | gRPC 接收 TradeRequest 且 Sync Sentinel 通过 | 创建 `trade_tasks` 记录，分配 `internal_tx_id` | 重启后扫描：重新进入仿真流程 |
| **SIMULATED** | 三源预言机 + EVM 仿真 + 不变性断言全部通过 | 持久化仿真结果（预期 Gas、输出金额） | 重启后扫描：重新仿真，校验当前链上水位线是否已变化 |
| **PENDING_APPROVAL** | 单笔价值超过大额审批阈值（见 §0.6） | 通知管理员，FSM 挂起等待 | 重启后：继续等待，不重新提交仿真 |
| **SIGNED** | MPC 签名完成，Nonce 已分配 | 持久化 `nonce` + `signed_payload`，**严禁重签** | 重启后：若 `tx_hash IS NULL`，直接用原 signed_payload 重广播 |
| **BROADCASTED** | Signed Payload 推送至 RPC 或 Flashbots | 持久化 `tx_hash` | 重启后：用 `tx_hash` 查询收据；超时触发 Gas 加速 |
| **CONFIRMED** | 链上达到 `tx_confirmation_blocks` 个安全确认 | 触发 §4.1 双分录对账，释放 Nonce 锁 | — （终态，已完成） |
...
func (m *RecoveryManager) detectAndHandleReorg(ctx context.Context, task *TradeTask) {
    ...
    if confirmedBlock.Hash != task.ConfirmedBlockHash {
        ...
        // 状态回滚至 SIMULATED，等待重新评估
        // 不直接回滚账本：由 §4.1 对账模块检测到差异后处理
