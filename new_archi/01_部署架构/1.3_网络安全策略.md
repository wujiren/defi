...
传输内容：**未签名的完整明文 Payload**（至少包含 `ChainID`, `To`, `Value`, `Data`, `Nonce`）。
*安全要求：由 Zone C 的 Proxy 接收明文并完成白名单校验无误后，再由 Zone C 内部自行组装并进行 Keccak256 Hash 计算，最后交由 MPC 分片签名。绝不信任 Zone B 传来的 Hash。*
